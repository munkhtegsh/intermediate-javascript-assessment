...
function callBinding(magicAnimals, updateAnimal, id) {
  let a =  magicAnimals.filter(item => {
    if (item.id === id) {
      return item;
    }
  });
  return updateAnimal.call(a[0], 'Trogdor');
}
...
function applyBinding(magicAnimals, updateAnimal, id) {
  let a =  magicAnimals.filter(animal => {
    if (animal.id === id) {
      return animal
    }
  })

  return updateAnimal.apply(a[0], ['being majestic', 'eating rainbows'] )
}
...
function emailList($q, $http) {
  ...
  var promise = new Promise(function (resolve, reject) {
    $http.get('/api/users').then(respond => {
      var emails = respond.data.map(item => {
      return item.email;
      })
    })
